                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <form action="{{ route('schedules.search') }}" method="GET">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="search" id="searchInput"
                                                    placeholder="Masukkan Bus / Terminal Tujuan" value=""
                                                    size="30">
                                    ...
                                    <div class="d-flex">
                                        <a href="{{ route('schedules.index') }}" id="refreshPage"
                                            class="btn btn-outline-info mr-2" data-toggle="tooltip" data-placement="top"
                                            title="Segarkan">
                                            <i class="fas fa-sync-alt mr-1"></i>
                                        </a>
                                        @if (Auth::user()->hasRole('Root'))
                                            <a href="{{ route('schedules.create') }}" class="btn btn-outline-success mr-2"
                                                data-toggle="tooltip" data-placement="top" title="Tambah">
                                                <i class="fas fa-plus"></i>
                                            </a>
                                            <a href="#" id="deleteAllSelectedRecord"
                                                class="btn btn-outline-danger mr-2" data-toggle="modal"
                                                data-target="#confirmationModal"
                                                data-url="{{ route('schedules.destroy.multi') }}">
                                            ...
                                            @foreach ($schedules as $schedule)
                                                <tr class="text-center" id="upt_ids{{ $schedule->id }}">
                                                    <td><input type="checkbox" name="ids" class="checkbox_ids"
                                                            id="{{ $schedule->id }}" value="{{ $schedule->id }}"></td>
                                                    <td>{{ $schedule->id }}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @if (auth()->user()->hasRole('Admin'))
                                                            ...
                                                            @if (!$isDisabled)
                                                                <div class="btn-group" role="group"
                                                                    aria-label="Basic example">
                                                                    <a href="{{ route('reservations.create', ['schedule_id' => $schedule->id]) }}"
