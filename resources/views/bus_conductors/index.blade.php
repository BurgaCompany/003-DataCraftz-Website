                                    ...
                                    <div class="d-flex">
                                        ...
                                        @if (Auth::user()->hasRole('PO'))
                                            ...
                                            <a href="#" id="deleteAllSelectedRecord" class="btn btn-outline-danger"
                                                ...
                                                data-url="{{ route('bus_conductors.destroy.multi') }}">
                                            ...
                                            @foreach ($bus_conductors as $bus_conductor)
                                                <tr class="text-center" id = "upt_ids{{ $bus_conductor->id }}">
                                                    <td><input type="checkbox" name="ids" class = "checkbox_ids"
                                                            id ="{{ $bus_conductor->id }}" value="{{ $bus_conductor->id }}">
                                                    </td>
                                                    ...
                                                    <td>{{ $bus_conductor->created_at->format('d-m-Y') }}<br>{{ $bus_conductor->created_at->format('H:i:s') }}
                                                    </td>
                                                    <td>
                                                        <div class="btn-group" role="group" aria-label="Basic example">
                                                            <a href="{{ route('bus_conductors.detail', $bus_conductor->id) }}"
                                                        ...
                                                        @if (Auth::user()->hasRole('PO'))
                                                            <div class="btn-group" role="group"
                                                                ...
                                                                <a href="{{ route('bus_conductors.edit', $bus_conductor->id) }}"
                                ...
                                <ul class="pagination justify-content-center">
                                    @if ($bus_conductors->previousPageUrl())
                                        <li class="page-item"><a class="page-link"
                                                href="{{ $bus_conductors->previousPageUrl() }}">Kembali</a></li>
                                    @else
                                    ...
                                    @for ($i = 1; $i <= $bus_conductors->lastPage(); $i++)
                                        <li class="page-item {{ $bus_conductors->currentPage() == $i ? 'active' : '' }}">
                                            <a class="page-link"
                                                href="{{ $bus_conductors->url($i) }}">{{ $i }}</a>
                                        </li>
                                    @endfor

                                    @if ($bus_conductors->nextPageUrl())
                                        <li class="page-item"><a class="page-link"
                                                href="{{ $bus_conductors->nextPageUrl() }}">Berikutnya</a></li>
                                    @else
